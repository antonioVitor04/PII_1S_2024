...
public class ProfessorDAO {
    ...
    public Professor [] obterProfessor () throws Exception{
        String sql = "SELECT nome_professor, id_professor FROM tb_professor";
        try (Connection con = ConexaoBD.obterConexao();
        ...
        while (rs.next()){
            ...
            professores[contador++] = new Professor (codigo, nomeProfessor);
        }
        return professores;
    }
    }
    
    public int obterCodigo(Professor u) throws Exception{
        int codigo = -1; 
        String sql = "SELECT id_professor FROM tb_professor WHERE nome_professor = ?";
        try(Connection con = ConexaoBD.obterConexao();
                PreparedStatement ps = con.prepareStatement(sql)){
            ps.setString(1, u.getNome());  
        try(ResultSet rs= ps.executeQuery()){
        if (rs.next()) {
            codigo = rs.getInt("id_professor");
        } 
        rs.close();
        ps.close();
        con.close();
    ...
    public Professor obterProfessorLogado (Professor u) throws Exception{
        ...
        Professor professor = new Professor (codigo, nomeProfessor, senha);
